...
public class FilteredDemagnetizationCoilContainerScreen extends BaseDemagnetizationCoilContainerScreen<FilteredDemagnetizationCoilContainer> {
    ...
    protected void renderForeground(int mouseX, int mouseY, DemagnetizationCoilBlockEntity entity){
        ...
        ScreenUtils.drawString(TextComponents.translation("simplemagnets.gui.demagnetization_coil.range", (rangeX - 1) * 2 + 1, (rangeY - 1) * 2 + 1, (rangeZ - 1) * 2 + 1).get(), 8, 26);
        ScreenUtils.drawCenteredString(TextComponents.string("x:").get(), 15, 51);
        ScreenUtils.drawCenteredString(TextComponents.number(rangeX).get(), 29, 52);
        ScreenUtils.drawCenteredString(TextComponents.string("y:").get(), 52, 51);
        ScreenUtils.drawCenteredString(TextComponents.number(rangeY).get(), 66, 52);
        ScreenUtils.drawCenteredString(TextComponents.string("z:").get(), 89, 51);
        ScreenUtils.drawCenteredString(TextComponents.number(rangeZ).get(), 103, 52);
