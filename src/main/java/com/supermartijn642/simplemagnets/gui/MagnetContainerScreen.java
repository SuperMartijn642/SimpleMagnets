...
public class MagnetContainerScreen extends ItemBaseContainerWidget<MagnetContainer> {
    ...
    @Override
    protected void renderForeground(MatrixStack poseStack, int mouseX, int mouseY, ItemStack stack){
        ScreenUtils.drawCenteredString(poseStack, TextComponents.item(stack.getItem()).get(), this.width() / 2f, 6);
        ScreenUtils.drawString(poseStack, ClientUtils.getPlayer().inventory.getName(), 32, 102);
    ...
    @Override
    protected void renderTooltips(MatrixStack poseStack, int mouseX, int mouseY, ItemStack stack){
