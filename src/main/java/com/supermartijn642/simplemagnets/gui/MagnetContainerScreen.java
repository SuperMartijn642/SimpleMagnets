...
public class MagnetContainerScreen extends ItemBaseContainerWidget<MagnetContainer> {
    ...
    @Override
    protected ITextComponent getNarrationMessage(ItemStack stack){
    ...
    @Override
    protected void renderBackground(int mouseX, int mouseY, ItemStack stack){
    ...
    @Override
    protected void renderForeground(int mouseX, int mouseY, ItemStack stack){
        ScreenUtils.drawCenteredString(TextComponents.item(stack.getItem()).get(), this.width() / 2f, 6);
        ScreenUtils.drawString(ClientUtils.getPlayer().inventory.getName(), 32, 102);

        ScreenUtils.drawCenteredString(TextComponents.translation("simplemagnets.gui.magnet.items").get(), 58, 24);
        ScreenUtils.drawCenteredString(TextComponents.translation("simplemagnets.gui.magnet.xp").get(), 166, 24);
        ScreenUtils.drawCenteredString(TextComponents.translation("simplemagnets.gui.magnet.filter").get(), 112, 68);

        ScreenUtils.drawCenteredString(TextComponents.number(this.itemRange).get(), 79.5f, 43);
        ScreenUtils.drawCenteredString(TextComponents.number(this.xpRange).get(), 187.5f, 43);

        super.renderForeground(mouseX, mouseY, stack);
    }
    ...
    protected void renderTooltips(int mouseX, int mouseY, ItemStack stack){
        if(mouseX > 79.5f - 6 && mouseX < 79.5f + 5 && mouseY > 43 - 2 && mouseY < 43 + 9)
            ScreenUtils.drawTooltip(TextComponents.translation("simplemagnets.gui.magnet.items.range", this.itemRange).get(), mouseX, mouseY);
        if(mouseX > 187.5f - 6 && mouseX < 187.5f + 5 && mouseY > 43 - 2 && mouseY < 43 + 9)
            ScreenUtils.drawTooltip(TextComponents.translation("simplemagnets.gui.magnet.xp.range", this.xpRange).get(), mouseX, mouseY);

        super.renderTooltips(mouseX, mouseY, stack);
    }
}
