...
public class ItemSpawnHandler {
    ...
    public static void add(DemagnetizationCoilBlockEntity entity){
        ...
        handler.blocks.putIfAbsent(entity.getLevel().getDimension().getType(), new LinkedList<>());
        handler.blocks.get(entity.getLevel().getDimension().getType()).add(new WeakReference<>(entity));
    }
    ...
    public static void onEntitySpawn(EntityJoinWorldEvent e){
        ...
        handler.blocks.putIfAbsent(e.getWorld().getDimension().getType(), new LinkedList<>());

        List<WeakReference<DemagnetizationCoilBlockEntity>> toRemove = new ArrayList<>();

        List<WeakReference<DemagnetizationCoilBlockEntity>> list = handler.blocks.get(e.getWorld().getDimension().getType());
