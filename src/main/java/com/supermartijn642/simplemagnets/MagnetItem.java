    ...
    public void inventoryUpdate(ItemStack stack, World level, Entity entity, int itemSlot, boolean isSelected){
        ...
        if(tag.contains("active") && tag.getBoolean("active")){
            if(this.canPickupItems(tag)){
                ...
                List<Entity> items = level.getEntities(EntityType.ITEM, area,
                    item -> item instanceof ItemEntity && item.isAlive() && (!level.isClientSide || item.tickCount > 1) &&
                        (((ItemEntity)item).getThrower() == null || !((ItemEntity)item).getThrower().equals(entity.getUUID()) || !((ItemEntity)item).hasPickUpDelay()) &&
                ...
                items.forEach(item -> item.setPos(entity.x, entity.y, entity.z));
                ...
                if(!level.isClientSide && entity instanceof PlayerEntity && CommonUtils.isModLoaded("itemphysic"))
                    items.forEach(item -> playerTouch((ItemEntity)item, (PlayerEntity)entity));
            }
