...
public abstract class MagnetItem extends BaseItem {
    ...
    private static void playerTouch(ItemEntity itemEntity, Player player){
        if(!itemEntity.level().isClientSide){
            ...
            if((result.isTrue() || (!itemEntity.hasPickUpDelay() && (itemEntity.getTarget() == null || itemEntity.lifespan - itemEntity.getAge() <= 200 || itemEntity.getTarget().equals(player.getUUID()))))
