...
public class DemagnetizationCoilBlock extends Block {
    ...
    public DemagnetizationCoilBlock(String registryName, Supplier<Integer> maxRange, Supplier<Boolean> hasFilter, Supplier<? extends DemagnetizationCoilTile> tileSupplier){
        super(Properties.create(Material.IRON, MaterialColor.GRAY).harvestTool(ToolType.PICKAXE).harvestLevel(0).setRequiresTool().hardnessAndResistance(3.0F, 5.0F).sound(SoundType.METAL));
    ...
    public void addInformation(ItemStack stack, IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn){
        if(this.hasFilter.get())
            tooltip.add(new TranslationTextComponent("simplemagnets.demagnetization_coil.info.filtered", this.maxRange.get()).mergeStyle(TextFormatting.AQUA));
        else
            tooltip.add(new TranslationTextComponent("simplemagnets.demagnetization_coil.info.no_filter", this.maxRange.get()).mergeStyle(TextFormatting.AQUA));
